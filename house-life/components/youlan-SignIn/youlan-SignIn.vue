	...
	<view class="calendar-box">
		<view class="top">
			<view>
				...
				<p v-if="!is_day_signin && isIntegral">今日签到可获得+{{ integral }}天会员</p>
		...
		<view class="but">
			...
			<!-- #endif -->
			<!-- #ifdef MP-TOUTIAO -->
			<button :disabled='thisMonth == month ? false : true'  :style="'background: ' + (is_day_signin ? signin_but_bg : (thisMonth == month ? bgday : signin_but_bg)) + ';'" @click="daySign(dayArr[aheadDay + day - 1])">看广告并签到</button>
			<!-- #endif -->
		</view>
	</view>
</template>
