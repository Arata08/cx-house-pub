...
<!-- 						<u-form-item label-width="150" :label-position="labelPosition" label="房间号" required>
						...
						<u-form-item label-width="150" :label-position="labelPosition" label="房屋面积" required>
							<u-input :border="border" placeholder="请输入房屋面积" v-model="model.houseArea" type="number"></u-input>
						</u-form-item>
						</u-form-item>
						...
						<u-form-item label-width="150" :label-position="labelPosition" label="楼层" required>
							<u-input :border="border" placeholder="请输入楼层" v-model="model.roomType" type="number"></u-input>
						</u-form-item>
						<u-form-item  :label-position="labelPosition" label="房间号" prop="houseNo" label-width="180"
						left-icon="home" :leftIconStyle="{color:'#d5d5d5'}">
							<u-input :border="border" placeholder="请输入房间号(必填)" v-model="model.houseNo" type="text"></u-input>
						</u-form-item>
						<u-form-item  :label-position="labelPosition" label="装修" prop="decoration" label-width="180" left-icon="eye" :leftIconStyle="{color:'#d5d5d5'}">
							<u-input :border="border" type="select" placeholder="请选择装修(必选)":select-open="decorationShow" v-model="decoration"  @click="decorationShow = true"></u-input>
						</u-form-item>
						...
						left-icon="rmb-circle" :leftIconStyle="{color:'#d5d5d5'}">
							<u-input :border="border" type="select" :select-open="actionSheetShow" v-model="payType" placeholder="请选择付款方式(必选)" @click="actionSheetShow = true"></u-input>
						</u-form-item>
...
export default {
	...
	methods: {
		...
		actionSheetCallback(index) {
			uni.hideKeyboard();
			this.directionVo = this.actionSheetList[index].text;
			this.payType = this.actionSheetList[index].text;
			this.model.payType = this.actionSheetList[index].text;
		},
		submit() {
			if(this.$u.test.isEmpty(this.model.roomType)){
				return this.$mytip.toast('请输入楼层')
			}
