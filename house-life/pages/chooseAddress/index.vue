	...
	<view class='app'>
		...
		<view class="current-address row b-b">
			...
			<image class="ser-icon"
				...
				@click="navTo('search?city='+curCity)"></image>
		</view>
		<!-- 补充详细地址 -->
		...
		<map id="map" class="map" :scale="15" :show-location="true" :longitude="map.longitude" :latitude="map.latitude"
		...
		<scroll-view scroll-y class="addr-list-scroll">
			<view class="addr-list">
				<view class="addr-item b-b row" v-for="(item, index) in list" :key="index"
